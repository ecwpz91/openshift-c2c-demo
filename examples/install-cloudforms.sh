...
function cfme.install {
 ...
 # [TODO] Check if required ServiceAccount and permisions are granted.
 # If not, call metrics-infra.install
 metrics-infra.install

 oc new-project cfme --description='Cloudforms Project' --display-name='cloudforms'

 # Configure security context constraints (SCCs) for your OpenShift service accounts
 oc adm policy add-scc-to-user anyuid system:serviceaccount:cfme:cfme-anyuid --as=system:admin
 oc adm policy add-scc-to-user privileged system:serviceaccount:cfme:cfme-orchestrator --as=system:admin
 oc adm policy add-scc-to-user privileged system:serviceaccount:cfme:cfme-httpd --as=system:admin
 oc adm policy add-scc-to-user privileged system:serviceaccount:cfme:cfme-privileged --as=system:admin
 ...
 oc policy add-role-to-user view system:serviceaccount:cfme:cfme-orchestrator -n cfme
 oc policy add-role-to-user edit system:serviceaccount:cfme:cfme-orchestrator -n cfme
